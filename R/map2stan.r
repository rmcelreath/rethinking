...
#'
#' @param flist A formula or list of formulas that define the likelihood and
...
#' values
#' @param pars Optional: character vector of parameters to return samples for
#' @param constraints Optional: named list of custom parameter constraints,
#' using Stan notation
#' @param types Optional: named list of custom parameter types, using Stan
#' notation
#' @param sample If \code{FALSE}, builds Stan code without sampling
#' @param iter Number of iterations of sampling. By default, half of these
#' iterations are warmup.
#' @param warmup Number of warmup iterations. By default, half of \code{iter}.
#' @param chains Number of independent chains to sample from
#' @param debug If \code{TRUE}, prints various internal steps to help with
#' debugging
#' @param verbose If \code{TRUE}, prints extra progress messages.
#' @param WAIC When \code{TRUE}, computes WAIC after sampling, storing the
#' result
