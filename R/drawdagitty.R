...
drawdagitty <- function( x , col_arrow="black" , col_segment="black" , col_labels="black" , cex=1 , lwd=1.5 , goodarrow=TRUE , xlim , ylim , shapes , col_shapes , radius=3 , add=FALSE , xkcd=FALSE , latent_mark="c" , ... ){ 
    require(dagitty)
    x <- as.dagitty( x )
...
drawopenpaths <- function( x , Z=list() , col_arrow="red" , ... ) {
    ...
    for ( i in 1:length(path_list) ) {
        if ( path_list$open[i]==TRUE ) {
            ...
            drawdagitty( path_dag , col_arrow=col_arrow , add=TRUE , ... )
        }
    }#i
...
drawdagitty( plant_dag , cex=1.2 , col_labels=c("red","black","red","red") , col_arrow=c("red","black","red") , goodarrow=TRUE )
...
coordinates( exdag ) <- list( x=c(z=0,x=1,y=2,U=1.5) , y=c(z=0,x=0,y=0,U=-1) )
drawdagitty( exdag , xkcd=TRUE , lwd=1.5 )
